...
def test_same_seed_replays_exactly() -> None:
    config = SyntheticClassificationStreamConfig(n_features=4, n_functions=3, n_classes=3, q_max=2, p_sample_min_x=0.3, p_sample_max_x=0.3)
...
def test_labels_match_noisy_score_argmax() -> None:
    trajectory = generate_classification_trajectory(
        SyntheticClassificationStreamConfig(n_features=6, n_functions=4, n_classes=3),
        50,
        seed=11,
...
def test_npz_round_trip(tmp_path: Path) -> None:
    trajectory = generate_classification_trajectory(
        SyntheticClassificationStreamConfig(n_features=5, n_functions=3, n_classes=4),
        10,
        seed=2,
    )
...
def test_prequential_evaluation_runs() -> None:
    trajectory = generate_classification_trajectory(
        SyntheticClassificationStreamConfig(n_features=8, n_functions=3, n_classes=3),
        30,
        seed=0,
    )
