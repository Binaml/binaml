...
$x_t\in\{0,1\}^d$ and the same boolean function family
...
function of the observed coordinates and the MLP baseline wraps scikit-learn's
\texttt{MLPRegressor} with its \texttt{sgd} solver.  For classification, the
linear SGD baseline uses softmax cross-entropy on centered binary features and
the MLP baseline wraps scikit-learn's \texttt{MLPClassifier}.  Scikit-learn
baselines are optional and are installed through the \texttt{benchmarks}
dependency extra so ordinary library users do not need scikit-learn.
...
version, and separate total, prediction, and observation timings; aggregate
...
but their errors are excluded from reported regression MSE and RMSE or
classification accuracy.  Time-series plots retain the full trajectory and
